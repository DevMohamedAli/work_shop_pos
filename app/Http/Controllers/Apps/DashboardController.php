    ...
    public function __invoke(Request $request)
{
    ...
    $chart_best_products = DB::table('product_transaction_details')
        ->select('products.title as title', DB::raw('SUM(product_transaction_details.qty) as total'))
        ->join('products', 'products.id', '=', 'product_transaction_details.product_id')
        ->groupBy('product_transaction_details.product_id')
        ->orderBy('total', 'DESC')
        ->limit(5)
        ->get();
